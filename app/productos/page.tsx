...
export default function ProductosPage() {
  ...
  const [importKind, setImportKind] = useState<"venta" | "costos">("venta");
  ...
  async function confirmImport() {
    ...
    if (importKind === "venta") {
      ...
      const { data, error } = await supabase.rpc("import_costs", { p_items: costItems });
      setImporting(false);
      if (error) return setError(error.message);
  ...
  const willDelete = importKind === "venta" && importMode === "replace"
    ? salePreview?.existing_total ?? 0
    : 0;
    ...
    <main className="max-w-6xl mx-auto p-8 pb-24">
      ...
      {showImport && (
        <div className="border border-gray-200 rounded-lg p-4 mb-6">
          <p className="text-sm font-medium mb-3">Importar desde archivo</p>

          <div className="flex gap-2 mb-4">
            {(
              [
                ["venta", "Lista de venta", "codigo, descripcion, marca, grupo, contado, credito"],
                ["costos", "Lista de compra", "codigo, compra, descuento (opcional)"],
              ] as const
            ).map(([k, label, cols]) => (
              <button
                key={k}
          ...
          {importKind === "venta" ? (
            ...
            <div className="mb-4">
              <p className="text-xs text-gray-500 mb-2">
                ...
                archivo no trae columna de descuento, cada producto conserva el suyo y puedes
                aplicarlo despues desde la vista Compra.
              </p>
              <label className="text-xs text-gray-500 block mb-1">Archivo CSV</label>
              <input type="file" accept=".csv,.txt" onChange={handleFile} className="text-sm" />
            </div>
